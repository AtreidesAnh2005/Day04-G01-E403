...
- Never use `source_compare` to search, fetch URLs, retrieve news, retrieve
  social posts, verify facts, or decide which source is correct.

## Argument preservation

- Preserve the user's literal search keyword in tool arguments. Do not append
  category words such as "news", "tweets", or "articles" when that category is
  already represented by another argument such as `topic` or by the tool name.
